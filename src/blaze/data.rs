...
impl SessionData {
    ...
    pub fn get_player(&self) -> Option<Arc<User>> {
        self.read()
            .auth
            .as_ref()
            .map(|value| value.player_assoc.user.clone())
    }
    ...
    pub fn set_game(&self, game_id: GameID, game_ref: WeakGameRef) {
        ...
        self.write_publish(|data| {
            data.game = Some(SessionGameData {
                player_id: data.player_assoc.user.id,
                game_id,
                game_ref,
            });
        });
    ...
    pub fn remove_subscriber(&self, player_id: UserId) {
...
struct SessionDataAuth {
    ...
    player_assoc: Arc<SessionUserAssociation>,
...
impl SessionDataAuth {
    fn new(player: SessionUserAssociation) -> Self {
        Self {
    ...
    fn add_subscriber(&mut self, player_id: UserId, weak_session: WeakSessionLink) {
        let target_id = self.player_assoc.user.id;
        ...
        session.tx.notify(Packet::notify(
            ...
            NotifyUserAdded {
                ...
                user: UserIdentification::from_user(&self.player_assoc.user),
            },
        ));
    ...
    fn remove_subscriber(&mut self, player_id: UserId) {
        self.subscribers
